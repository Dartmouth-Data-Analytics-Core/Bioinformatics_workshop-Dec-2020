...
Using quantities that we observe from our sample of the population (mean, standard deviation, sample size), and assuming our sample was generated by a distribution with known area such as the t-distribution, we can calculate a test-statistic using the equation (for the t-distribution):
